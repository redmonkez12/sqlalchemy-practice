...
def task_07_core(engine, create_tables):
    visits = Table(
        "visits",
        metadata,
    ...
    transactions = Table(
        ...
        Column("visit_id", Integer, ForeignKey(visits.c.visit_id, ondelete="CASCADE", onupdate="CASCADE"),
               nullable=False),
    ...
    transactions.drop(engine, checkfirst=True)
    visits.drop(engine, checkfirst=True)
    create_tables()
